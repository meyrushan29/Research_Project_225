...
class CombinedResultScreen extends StatelessWidget {
  ...
  Widget _buildXaiHeatmapCard(String heatmapUrl, {String? description}) {
    final String fullImageUrl = "${AuthService.baseUrl}$heatmapUrl";
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              const Icon(Icons.remove_red_eye_outlined, color: Colors.orangeAccent, size: 20),
              const SizedBox(width: 12),
              Text(
                "VISUAL EXPLANATION",
                style: GoogleFonts.orbitron(
          ...
          ClipRRect(
            ...
            child: Image.network(
              ...
              errorBuilder: (_, __, ___) => Container(
                height: 100,
                color: Colors.black,
                child: const Center(child: Text("Heatmap unavailable", style: TextStyle(color: Colors.white24))),
              ),
            ),
          ),
          const SizedBox(height: 12),
          Text(
            description ?? "Highlighted areas show where the AI detected signs of dehydration.",
            style: GoogleFonts.exo2(color: Colors.white70, fontSize: 13),
            textAlign: TextAlign.center,
          ),
          const SizedBox(height: 8),
          Text(
            "Heatmap Scale: Red (Highest Focus) → Blue (Background)",
            style: GoogleFonts.exo2(color: Colors.white38, fontSize: 10, fontStyle: FontStyle.italic),
            textAlign: TextAlign.center,
          ),
        ],
      ),
