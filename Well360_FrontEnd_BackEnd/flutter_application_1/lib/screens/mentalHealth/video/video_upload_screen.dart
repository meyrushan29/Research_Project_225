...
class _VideoUploadScreenState extends State<VideoUploadScreen>
  ...
  Widget _buildResultsSection() {
    ...
    final totalFrames = (_analysisResult!['total_frames'] ?? 1) as num;
    final durationSec = (_analysisResult!['duration_sec'] ?? 0.0) as num;
    final videoQuality = (_analysisResult!['video_quality'] ?? 1.0) as num;
    final backendUsed = _analysisResult!['backend_used'] ?? 'cnn';
    ...
    return Column(
      ...
      children: [
        ...
        // Video Quality Warning (shown only when face visibility was low)
        _buildQualityIndicator(videoQuality.toDouble(), framesAnalyzed as int, totalFrames.toInt()),

        // Backend Badge (subtle, tech-savvy users appreciate this)
        Padding(
            ...
            children: [
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Icon(
                      backendUsed == 'fer' ? Icons.verified : Icons.memory,
                      size: 11,
                      color: backendUsed == 'fer' ? Colors.cyanAccent : Colors.white38,
                    ),
                    const SizedBox(width: 5),
                    Text(
                      backendUsed == 'fer' ? 'MTCNN + Deep Model' : 'CNN Fallback',
                      style: GoogleFonts.exo2(
                        fontSize: 10,
                        color: backendUsed == 'fer' ? Colors.cyanAccent.withValues(alpha: 0.7) : Colors.white38,
                        letterSpacing: 0.5,
                      ),
                    ),
                  ],
        ...
        if (recommendations.isNotEmpty) ...[
          const SizedBox(height: 24),
          _buildPersonalizationBadge(recContext),
          const SizedBox(height: 12),
  ...
  Widget _buildQualityIndicator(
      double qualityRatio, int framesAnalyzed, int totalFrames) {
    if (qualityRatio >= 0.4) return const SizedBox.shrink();

    final pct = (qualityRatio * 100).toInt();
    return Container(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  'Only $framesAnalyzed / $totalFrames frames had a detectable '
                  'face. For best results: use good lighting, face the camera '
                  'directly and reduce motion.',
                  style: GoogleFonts.exo2(
                      fontSize: 11,
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // NEW: Personalization Context Badge
  // ──────────────────────────────────────────────────────────────────────────
  Widget _buildPersonalizationBadge(Map<String, dynamic> ctx) {
    ...
    if (isFirst) {
      ...
      label = 'Personalised for your ${timeCtx[0].toUpperCase()}${timeCtx.substring(1)} mood';
