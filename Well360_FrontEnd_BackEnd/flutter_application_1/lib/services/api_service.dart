...
class ApiService {
  ...
  static void _handleError(Object e) {
    if (e is http.ClientException || e is SocketException) {
      throw Exception(
        ...
        "Check IP in Login Settings. "
        "${AuthService.connectionHint}"
      );
    } else if (e is TimeoutException || e.toString().contains('timeout')) {
      throw Exception("Request timed out. Backend at $baseUrl may be slow/busy.");
    } else {
      throw e;
    }
  }
  ...
  static Future<List<dynamic>> getFitnessHistory({int limit = 20}) async {
    return await _get("/predict/fitness/history?limit=$limit");
  }

  static Future<Map<String, dynamic>> getFitnessRecommendation() async {
    return await _get("/predict/fitness/recommendation");
  }

  static Future<Map<String, dynamic>> analyzeFitness() async {
    return await _post("/predict/fitness/analyze", {});
  }

  static Future<List<dynamic>> getFitnessNotifications({int limit = 10}) async {
    return await _get("/predict/fitness/notifications?limit=$limit");
  }

  static Future<Map<String, dynamic>> getFitnessProgress({String? exercise, int limit = 10, int? cycleNumber}) async {
    String url = "/predict/fitness/progress?limit=$limit";
    if (exercise != null && exercise.isNotEmpty) {
      url += "&exercise=$exercise";
    }
    if (cycleNumber != null) {
      url += "&cycle_number=$cycleNumber";
    }
    return await _get(url);
  }

  static Future<Map<String, dynamic>> getBodyPartRadar({int limit = 50}) async {
    return await _get("/predict/fitness/body-part-radar?limit=$limit");
  }

  static Future<List<dynamic>> getMuscleRadarHistory() async {
    return await _get("/predict/fitness/muscle-radar-history");
  }
